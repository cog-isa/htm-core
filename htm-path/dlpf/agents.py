        ...
        for i in wts:
            if len(i.shape) == 2:
                ...
                for c in range(i.shape[1]):
                    ...
                    if to_save:
                        plt.imsave('imgs/' + 'layer' + str(j) + '/' + to_save + '-neuron' + str(c) + '.png',
                                   neuron_input)
                    else:
                        plt.show()
            ...
            while len(unused) > 0:
                ...
                for i in batch_indexes:
                    for x in range(num_of_layers):
                        for y in range(width_of_layers):
                            for z in range(height_of_layers):
                                input_batch[j][x][y][z] = inputs[i][0][x][y][z]
                    j += 1
                j = 0
                for i in batch_indexes:
                    for x in range(self.number_of_actions):
