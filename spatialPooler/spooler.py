...
from spatialPooler.utils import kth_score
import random
...
class SpatialPooler:
    ...
    def out_prediction(self, region):
        output=[[0 for i in range(0, region.get_input_h())] for i in range(0, region.get_input_w())]
        for col in region.columns:
            if col.get_is_active() == True:
                for syn in col.get_potential_synapses():
                    x = syn.get_index_connect_to() / region.get_input_h()
                    y = syn.get_index_connect_to() % region.get_input_w()
